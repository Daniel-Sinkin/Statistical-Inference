...
We write
$$
a_k = (-1)^{k + 1} k \binom{n}{k}
$$

First observe that for
$$
\begin{aligned}
b_k := k \binom{n}{k} &= \frac{n!}{k!(n - k)!}k \\
&= \frac{n!}{(k - 1)!(n - k)!} \\
&= \frac{n!}{(k - 1)!(n - k + 1)!} (n - k + 1) \\
&= \binom{n}{n - k + 1} (n - k + 1) = b_{n - k + 1}
\end{aligned}
$$
holds and $a_k = (-1)^{k + 1} b_k$. By parity it follows that $a_k + a_{n - k + 1} = 0$

We can write for $n$ even
$$
\sum_{k = 1}^n a_k = \sum_{k = 1}^{n / 2} a_k + a_{n - k + 1} = 0,
$$
and for $n$ odd
$$
\sum_{k = 1}^n a_k = \sum_{k = 1}^{(n - 1) / 2} a_k + a_{n - k + 1} + a_{(n + 1) / 2} = a_{(n + 1) / 2}.
$$